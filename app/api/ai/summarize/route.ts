...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await AiService.generateSummary(validated.callId, session.user.id);
    ...
    return NextResponse.json(
      createErrorResponse(error instanceof Error ? error.message : "Failed to generate summary"),
      { status: 500 }
    );
  }
}
